...
| `@symma/protocol` (npm)             | ACP engine: framing, JSON-RPC peer, session driving, read-only permission floor, agent specs. Extracted from this repo; `src/shared/acp-protocol.ts` is the superseded copy, imported by nothing and deleted once the extraction completes |
| `src/shared/acp.ts`                 | ACP ReviewBackends over that engine: local spawn plus the shared backend surface both local and gateway-remote use                                                                                                                         |
