...
| `commandcode-access-key`     | No       | —                     | Used when the main or aux model names `commandcode`; accepts a comma-separated list — each run logs every key's meters, then picks the key with the largest share of its monthly plan remaining whose rate windows are open (all window-limited → fullest share; probes unavailable → first key) |
