...
| `context-pack`           | `diff-batches`, except that each page with a context pack (the code around each change, the definitions it uses, import-linked callers and a directory map) gets it before the first turn, in place of caller evidence and the changed-symbol usage list, under a pack-aware prompt | Experimental; scored offline by `npm run replay:context-pack` and by a live turn-count A/B before any default change. OpenCode reports supplied re-reads. |
