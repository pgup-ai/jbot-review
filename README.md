...
| `commandcode-access-key`     | No       | —                     | Used when the main or aux model names `commandcode`; accepts a comma-separated list — each run picks the key with the most monthly credits remaining whose rate windows are open (all window-limited → fullest key; probes unavailable → first key) |
