...
| `max-concurrent-sessions` | `4`                            | Max model sessions in flight (`0` = unlimited). Free/throttled tiers serialize one key's requests upstream — observed as a flash session queued 7+ minutes behind parallel shards. The capped default keeps each session's deadline measuring model time, not queue time; drop to 2–3 on tight free tiers, or set `0` on paid tiers with real concurrency.                                                                                                                                                                                                                                                                                                  |
...
max-concurrent-sessions: '0' # unlimited on a paid concurrent tier (default caps at 4)
