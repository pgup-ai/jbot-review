...
- **Shipped, default on:** grace-expiry abort of abandoned aux sessions
  ...
  CommandCode `--effort` delivery from a probed per-model allowlist
  (2026-08-22: deepseek-v4-flash accepts high|max, muse-spark none), with
  the one-knob mapping: an explicit `model-options` effort clamps to each
  model's nearest declared tier while built-in defaults deliver only on an
  exact match (`modelOptionsExplicit`), so default behavior is unchanged;
  pi aux sessions on a distinct model now receive the aux thinking level.
  Kilo's `--variant` never reaches `kilo acp` sessions and Devin encodes
  effort in the model id, so those stay pool-configured (delivery audit:
  docs/superpowers/audits/2026-08-22-reasoning-effort-delivery-audit.md).
...
| TASK-002 | P0 / Latency 0, Quality + / M | Create `scripts/review-benchmark.ts` and an `npm run benchmark:review` script. The runner must accept a manifest, run control and treatment with isolated temp homes/worktrees, capture exit status and telemetry, and write one JSON summary plus per-case JSONL without posting to GitHub.                               | TASK-001                     | Yes       | 2026-08-21 |
| TASK-003 | P0 / Latency 0, Quality + / M | Create `test/fixtures/review-benchmark/manifest.json` with schema version, case id, repository/ref or fixture path, base/head identifiers, risk tier, expected findings, expected-clean flag, and allowed alternative anchors. Keep secrets and proprietary source out of committed fixtures.                              | TASK-001                     | Yes       | 2026-08-21 |
| TASK-004 | P0 / Latency 0, Quality + / M | Add a deterministic scorer in `src/shared/benchmark-score.ts` with tests in `test/benchmark-score.test.ts`. Compute severity-weighted recall, precision, clean false-positive rate, anchor rate, duplicate rate, median/p90/p95 latency, cost per retained finding, and bootstrap confidence intervals using a fixed seed. | TASK-001                     | Yes       | 2026-08-21 |
...
| TASK-006 | P0 / Latency 0, Quality + / S | Add a benchmark comparability check that rejects a comparison when model, revision, engine, reasoning effort, sampling options, prompt version, or corpus hash differs outside the declared treatment variable.                                                                                                            | TASK-002                     | Yes       | 2026-08-21 |
...
| TASK-008 | P0 / Latency 0, Quality + / S | Add a plan gate: no default behavior change from later phases may merge without a benchmark report that references the treatment commit, corpus hash, model/config tuple, sample size, quality result, latency result, and rollback flag. Document the gate in the scorecard.                                              | TASK-001                     | Yes       | 2026-08-21 |

**Phase status:** The runner, corpus manifest, scorer, comparability check, and
merge gate are built (TASK-002/003/004/006/008). TASK-001's scorecard spec,
TASK-005's recorded baseline, and TASK-007's critical-path re-measurement are
outstanding, so comparisons still cite ad-hoc runs rather than a frozen
baseline.

**Merge gate (TASK-008).** No change that alters default review behavior merges
without a benchmark summary whose `mergeGate.satisfied` is true.
`checkBenchmarkMergeGate` in `src/shared/benchmark-report.ts` decides that, and
`review-benchmark` writes the verdict into every `summary.json` and warns when
it is unmet. The gate requires the treatment commit, corpus hash, rollback
instruction, a model/config tuple and successful-run count for both arms, an
adjudicated quality result, and a latency result whose sample can resolve some
effect. That last clause exists because Phase 3 produced a -28.2% p50 reading
from a sample that could resolve nothing.

The manifest supplies `treatmentCommit` and `rollback`, since neither is
...
| Task     | Priority / Impact / Effort    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Dependencies                 | Completed           | Date       |
| -------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------- | ---------- |
| TASK-028 | P1 / Latency H, Quality + / S | Derive `EMBEDDED_FIRST_REVIEW_PROMPT` from the `REVIEW_PROMPT` control in `src/shared/prompt.ts` so fully embedded hunks are authoritative and already read. Explicitly prohibit rerunning `git diff` or rereading changed code solely to reproduce embedded content.                                                                                                                                                                                                                    | TASK-005, TASK-009           | Yes                 | 2026-08-20 |
| TASK-029 | P1 / Latency H, Quality + / S | Add one canonical repository-exploration policy to the embedded-first variant in `src/shared/prompt.ts`: tools are for omitted/truncated hunk recovery, direct caller/callee/contract/test checks tied to a changed symbol, or evidence confirmation for a candidate finding. Remove or reconcile wording that describes all embedded hunks as merely a starting point.                                                                                                                  | TASK-028                     | Yes                 | 2026-08-20 |
...
| TASK-031 | P1 / Latency M, Quality + / S | Add `EMBEDDED_FIRST_PI_REVIEW_SYSTEM_PROMPT` alongside the `PI_REVIEW_SYSTEM_PROMPT` control so `git_diff` is used only for omitted/truncated coverage recovery, not automatically whenever later instructions mention diff inspection. Keep path-scoped diff retrieval as the preferred recovery form.                                                                                                                                                                                  | TASK-029                     | Yes                 | 2026-08-20 |
...
| TASK-035 | P1 / Latency H, Quality 0 / S | Run a prompt-only A/B with all code-side enforcement disabled. Graduate the prompt change only if tool-output bytes fall at least 25% on a backend that reports tool telemetry, and QLT-003/004 pass. Report p50 main-session latency without gating on it: the corpus carries a per-pair spread near 74 percentage points, so a 10% latency effect needs roughly 400 pairs while tool bytes resolve at 24. Size every sample so its minimum detectable effect clears whatever it gates. | TASK-034, TASK-019, TASK-025 | Yes (not graduated) | 2026-08-20 |
...
| Task     | Priority / Impact / Effort    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                               | Dependencies              | Completed                      | Date       |
| -------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------ | ---------- |
| TASK-036 | P0 / Latency M, Quality + / M | Add pure `src/shared/exploration-policy.ts` and `test/exploration-policy.test.ts`. Define `ExplorationMode` (`embedded`, `coverage-recovery`, `single-shot`), `ExplorationTier` (`minimal`, `standard`, `elevated`), and limits for ordinary calls, tool-output bytes, unique adjacent files, and repeat reads/searches. Dependency depth stays a prompt rule: a tool call carries no provenance tying it to the diff, so the boundary cannot count hops. | TASK-009, TASK-035        | Yes                            | 2026-08-21 |
| TASK-037 | P1 / Latency H, Quality 0 / S | Set initial experimental defaults: minimal = 4 ordinary calls/64 KiB/3 adjacent files; standard = 8/128 KiB/6; elevated = 16/256 KiB/12. Treat these as A/B constants, not public configuration, until Phase 4 graduation.                                                                                                                                                                                                                                | TASK-036                  | Yes                            | 2026-08-21 |
| TASK-038 | P0 / Latency 0, Quality + / M | Carry `DiffHunksBlockResult.truncatedFiles` and `omittedFiles` into each `ShardPlan`. Enter `coverage-recovery` when either list is non-empty; otherwise enter `embedded`. Add a pure assertion that every omitted/truncated reviewable file has a permitted recovery path.                                                                                                                                                                               | TASK-036                  | Yes                            | 2026-08-21 |
| TASK-039 | P0 / Latency 0, Quality + / M | Exempt only path-scoped diff recovery for the named omitted/truncated files from the ordinary-call budget. Cap each recovery response, record completion per file, reject recovery requests for unrelated paths, and switch to the normal tier after all named coverage gaps have been accessed.                                                                                                                                                          | TASK-038                  | Yes                            | 2026-08-21 |
| TASK-040 | P1 / Latency H, Quality 0 / M | Enforce counters in Pi's custom `read_file` and `git_diff` wrappers. On soft exhaustion, return a bounded tool result instructing the session to finish from current evidence; allow the final assistant response. On an attempted hard violation after soft stop, refuse that tool call without failing the session.                                                                                                                                     | TASK-036, TASK-039        | Yes                            | 2026-08-21 |
| TASK-041 | P1 / Latency H, Quality 0 / L | Add equivalent enforcement to ACP routes at the protocol/tool boundary when the capability matrix marks them enforceable. Keep permission denial distinct from budget exhaustion in telemetry.                                                                                                                                                                                                                                                            | TASK-016, TASK-036        | Yes (no enforceable ACP route) | 2026-08-21 |
| TASK-042 | P1 / Latency M, Quality 0 / M | Add enforcement to OpenCode only if SDK/session hooks can deny or replace an individual tool result. If the installed SDK is observe-only, use prompt policy plus timeout and mark the backend `observe-only`; do not claim enforcement.                                                                                                                                                                                                                  | TASK-013, TASK-036        | Yes (observe-only)             | 2026-08-21 |
| TASK-043 | P1 / Latency M, Quality 0 / M | Add Qoder enforcement only through a documented SDK hook proven by a focused contract test. Do not set `maxTurns` until successful-run turn distributions exist.                                                                                                                                                                                                                                                                                          | TASK-014, TASK-036        | Yes (no documented hook)       | 2026-08-21 |
| TASK-044 | P2 / Latency M, Quality 0 / S | For opaque CLI routes such as dim, inject the common prompt policy and retain the absolute timeout. Record that byte/path enforcement is unavailable rather than lowering a global turn ceiling.                                                                                                                                                                                                                                                          | TASK-015, TASK-036        | Yes (unavailable, recorded)    | 2026-08-21 |
| TASK-045 | P1 / Latency M, Quality 0 / S | After at least 30 successful sessions per backend/risk tier, calculate the p90 and p99 useful-turn distributions where a useful turn precedes a retained finding or resolves coverage. Add a backend-specific soft finish threshold at p90 and retain timeout/p99 only as a runaway guard.                                                                                                                                                                | TASK-019, TASK-036        | Deferred: needs 30 sessions    | 2026-08-21 |
| TASK-046 | P0 / Latency 0, Quality + / M | Add tests for budget exhaustion, coverage exemption, transition out of recovery, repeated reads, equivalent searches, soft finish, opaque backend behavior, failure classes, and full-diff coverage under every tier.                                                                                                                                                                                                                                     | TASK-038 through TASK-045 | Yes                            | 2026-08-21 |
...
**Phase status:** Implemented and inert by default. `src/shared/exploration-policy.ts`
owns the budget; only a backend whose capability is `enforceable` receives one,
which today is pi alone. opencode, qoder, dim, and local ACP report
`observable` and remote ACP reports `opaque`, so none of them can refuse a call
and none is described as enforcing a budget. TASK-047's graduation run is
outstanding; per the Phase 3 measurement work its tool-output-token arm is
resolvable on a small sample while its 15% p50 arm is not.
...
- Dependency depth stays a prompt rule rather than a counter: a tool call carries no provenance tying it to the diff, so the boundary cannot measure hops.
- TASK-045 is deferred until a cohort reaches 30 successful sessions. The calculation lands with the consumer that reads it; shipping it earlier would be surface nothing calls.
...
(changed files 8KiB, commits 4KiB, flat prior comments 8KiB with jbot's own
review bodies excluded, prior-threads 32KiB backstop) — invariant-#4
enforcement, default on; the guideline widen-fallback now keeps the finder
...
| Task     | Priority / Impact / Effort    | Description                                                                                                                                                                                                                                                                                        | Dependencies              | Completed                               | Date       |
| -------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------- | ---------- |
| TASK-076 | P1 / Latency H, Quality 0 / M | Add cancellation plumbing to `ReviewBackend` operations and `settleWithinGrace`. When a fail-open auxiliary result is abandoned at grace expiry, abort the underlying session, release its slot, record `aborted-after-grace`, and prevent process-exit linger.                                    | TASK-009                  | Yes (label-abort; pi+opencode)          | 2026-08-21 |
| TASK-077 | P1 / Latency H, Quality 0 / S | Implement Phase 8 cancellation first for Pi using its existing best-effort abort path and active-session registry. Add tests proving no post-review token row arrives from an abandoned session and teardown finishes promptly.                                                                    | TASK-076                  | Yes                                     | 2026-08-21 |
...
| TASK-079 | P1 / Latency H, Quality 0 / M | Prototype post-main verification overlapping only the remaining auxiliary settle grace. Snapshot settled findings after main, verify that snapshot concurrently, merge late findings under existing fail-open semantics, and record `late-unverified` counts. Keep this behind an experiment flag. | TASK-009, TASK-025        | Yes (flag, default off)                 | 2026-08-21 |
| TASK-080 | P0 / Latency 0, Quality + / S | Reject the overlap treatment automatically if any late unverified P0/P1 finding appears, if late-unverified blocking findings exceed 0.5% of runs, or if precision is worse than the serial verifier within QLT-004.                                                                               | TASK-079                  | Yes (signal; auto-reject at graduation) | 2026-08-21 |
...
**Phase status (2026-08-21):** TASK-076/077 shipped as a label-scoped
best-effort abort: `ReviewBackend.abortSessionsByLabel` (optional), fired by
`settleWithinGrace` settle-first at grace expiry, implemented for pi (existing
abandon primitive) and opencode (`session.abort`); CLI/ACP routes stay
wait-bounded (TASK-078 open). TASK-079/080 shipped behind
`JBOT_VERIFY_OVERLAP_GRACE` (default off): the main-settle snapshot is
verified concurrently with the graces and verdicts re-attach by path:line
...
| TASK-157 | P1 / Latency - risk, Quality + / S | Give verification its own reasoning effort instead of whatever the aux model entry happens to carry, and clamp an unsupported effort to the nearest supported tier rather than dropping it. Parity with the main pass is the floor.                                         | None                      | Yes       | 2026-08-21 |
...
Implemented 2026-08-21: `clampReasoningEffort`/`verificationModelOptions` in
`config.ts`; delivery is per-backend — pi passes a per-session thinking level,
and opencode (whose prompt API has no per-session options) registers a
config-time model alias `<model>--jbot-verify` whose `id` routes back to the
real model (probe-verified against the live gateway). The runner passes the
floored options only when the aux entry does not already deliver them.
...
### Implementation Phase 17 — Close `INC-001`'s oversized single-shard failure class
...
| Task     | Priority / Impact / Effort          | Description                                                                                                                                                                                                                                                                                                                                                                                                                                             | Dependencies                        | Completed | Date       |
| -------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | --------- | ---------- |
| TASK-146 | P0 / Latency 0, Quality + / S       | Add an immutable private benchmark manifest entry for the `INC-001` pull request, head `be6a37bd83004deef3a777f59b8fccd3e21e1b5f`. Record the 41-file/160,489-patch-character shape, expected full-diff coverage, known historical findings, failed workflow/job ids, and permitted local reproduction method without committing proprietary source.                                                                                                    | TASK-002 through TASK-005           |           |            |
...
| TASK-154 | P1 / Latency H, Quality + / S       | Graduate an incident fix only when the fixed-model `INC-001` reproduction completes within 30 minutes in all required repetitions, preserves every adjudicated P0/P1/P2 finding, posts no new clean false positive, and does not increase provider failure/rate-limit rate. Archive the control and treatment telemetry with the decision.                                                                                                              | TASK-149 through TASK-153           |           |            |
| TASK-155 | P1 / Latency H waste, Quality 0 / S | Before any main-shard retry whose first attempt exceeded 60 seconds, re-fetch the PR state and current head through the existing GitHub client. If the PR is merged/closed or the head no longer equals the reviewed head, classify the run as `stale-before-retry`, skip the retry, post nothing, and finish telemetry without treating stale cancellation as a code-review finding. Local mode bypasses this check.                                   | TASK-011, TASK-150                  | Yes       | 2026-08-21 |
...
**Phase status (2026-08-21):** TASK-150 shipped as the pure
`classifyMainShardFailure` policy (`retry-policy.ts`): auth, model-not-found,
and context-length failures never retry; unknown stays retryable
(recall-safe); the budget fit check is unchanged. TASK-155 shipped: a retry of
a >60s attempt re-checks PR state (`getPullFreshness`) and a merged, closed,
or moved PR abandons the run as `stale-before-retry` (terminal state
`skipped`, nothing posted); local mode bypasses. The remaining Phase 17 tasks
(fixture, preflight envelope, auto-shard, adaptive reshard) are open.

Completion criteria for Phase 17:

- The `INC-001` shape is reproducible without relying on an expiring Depot artifact.
...
- **FILE-034**: Private benchmark manifest entry for `INC-001` — immutable head metadata, expected findings, and content hashes without proprietary source.
