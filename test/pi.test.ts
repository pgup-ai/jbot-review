...
describe('Pi review sessions', () => {
  ...
  it('wraps up a cut-off review in the same session with tools off and marks it partial', async () => {
    ...
    assert.deepEqual(events, ['prompted:4', 'aborted', 'tools:', 'prompted:0', 'disposed']);
    assert.equal(
      finalizePiSessionsByLabel(runtime, 'review', () => {}, 20_000),
      0,
    );
  ...
  it('delivers the verifier effort as a per-session thinking level', async () => {
    ...
    assert.deepEqual(sessions[0]?.tools, ['read', 'grep', 'find', 'ls']);
  ...
  it('reports the SDK-exposed turn count with observable native tool capability', async () => {
    ...
    runtime.sdk.createAgentSession = async (options) => {
      ...
      result.session.subscribe = (fn) => {
        listener = fn;
        return () => {};
      };
      const prompt = result.session.prompt;
      result.session.prompt = async (...args) => {
        listener({
          type: 'tool_execution_start',
          toolCallId: 'read-1',
          toolName: 'read',
          args: { path: 'value.ts', offset: 1, limit: 10 },
        });
        listener({
