...
**Status:** Approved design (2026-06-28). Implementation plan to follow via
...
*whole* diff's shape; it is a no-op on any non-trivial PR (full tier). This work
adds a second, finer reducer keyed off the *incremental* delta and gating
*per session* rather than by count. Motivated by the latency diagnosis in
...
structural — the run *blocks* on the slowest lens settling.
...
finding **verification** pass (invariants #1, #3). The gate only ever *reduces*
...
- **`interactions` keys on exported-symbol change**, not delta size — its whole
  job is "a *small* change breaks *unchanged* callers," so size is the wrong
...
`fanout.ts` already *is* the recall-supplement fan-out policy ("Scale
...
No other call sites change. The filter sits *after* the existing selection, so
...
whole-diff **ceiling** (its minimal tier still zeroes lenses for a tiny *fresh*
...
## Safety — every fallback fails toward *more* coverage
...
- **Accepted residual:** we cannot tell whether a prior lens run *failed open*
...
- **Escape hatch:** rides on the existing `dynamic-fanout` input — no new flag.
  ...
  governs them: `dynamic-fanout=true` applies whole-diff tiering *and*
...
1. **Full-diff scope, always (#1).** Main review + verification cover the
   complete `base...head` every push. The gate touches only the *number* of
   ...
   since the last review" now also informs lens *selection*, not just the
...
2. **Auxiliary sessions fail open (#3).** Skipping is a deliberate recall trade,
   the requested config is the ceiling, and every fallback runs *more* lenses.
...
A runner-level test asserts the wiring never narrows the main shards or
verification (invariant #1) and that the gate is bypassed when
`dynamic-fanout=false`.
